...
<table>
<tr>
<td align='left' class='tablas_items' > <?php echo $row['nombre']; ?></td>
</tr>
<tr>
<td align='left'> Precio:</td><td align='left' class='precio'><? echo $row['precio']."�" ?> </td>
</tr>
</table>
<?php
if ($cont==3)
...
$conn = db_connect();
	...
	echo "No hay ning�n pedido que concuerde con esa referencia de pedido, Gracias<br> ";
	else
	{
...
<tr><td width="68%" colspan='4'   ><? echo  utf8_encode($result['name']) ;?> <? echo  " ".utf8_encode($result['surname']) ;?></td>
</tr>
<tr><td width="68%" colspan='4'  ><? echo  utf8_encode($result['address']);?>          </tr>
<tr><td colspan='5'  ><? echo $result['city'];?>  -<?php echo  $result['zip'];?></td></tr>
<tr><td colspan='4'  ><?php echo utf8_encode($result['state']);?></td></tr>
<tr><td colspan='4'  ><?php echo  utf8_encode($result['country']);?></td></tr>
<tr><td class='datos_envio' colspan='4' > <?php if ($result['telf']!=0) echo $result['telf']; ?>
 Tel&eacute;fono: <?php echo  $result['movil'];?>            </td>
</tr>
<tr><td class='datos_envio' colspan='4' > Modo de pago:<?php if  ($result['modo']==1)
echo "Transferencia o ingreso";
else
echo "Contrareembolso"; ?></td>
</tr>
<tr>
...
//Funci�n para saber referencia y datos del art�culo
...
if ( !session_is_registered("arts_unidades_array") )
session_register("arts_unidades_array");
...
{
?>
<td width="30%" class="items"><div align="center"><? echo utf8_encode($row['nombre']) ;?></div></td>
<?php
}
else
{
?>
<td width="10%" class="items"><div align="center"><?php echo $_POST['name'] ?>/div></td>
<?php
}
if ( !$_POST['unidades'] )
{
?>
<td width="20%"><div align="center"><?php echo $unidades ?></div></td>
<?php
}
else
{
?>
<td width="20%"><div align="center"><?php echo $_POST['unidades']; ?></div></td>
<?php
}
if ( !$_POST['price'] )
{
...
{
?>
<td width="20%"><div align="center"><?  echo redondear_dos_decimal($arts_datos['art_price']);?></div></td>								
<?php
}
if ( !$_POST['price_tot'] )
{
?>
<td width="20%"><div align="center"><?  echo redondear_dos_decimal($arts_datos['art_price']*$unidades); ?></div></td>
<?php
}
else
{
?>
<td width="20%"><div align="center"><?php echo $_POST['price_tot']*$unidades; ?>"</div></td>
<?php
}?>
</tr>
...
$iva=$total_pagar - $base_imponible;
	...
	//dejo abierto la posibilidad del RE para mas adelante
	/*if ($_SESSION['re'] )
	{
	?>
	<td  align='left' >RE : <?php echo $_SESSION['re']." �"; ?> </td>
	<?php
	}*/
	?>
	<tr>
...
<table border='0' align='center' width="100%">
 ...
 <tr>
 <td align='left' ><font size="-4" >La mercancía enviada por Gloria Botonero mediante la agencia Tourline Express está asegurada. Cualquier rotura o desperfecto causado durante el transporte, deberá ser comunicado a Gloria Botonero en un plazo máximo de 24 horas desde la recepción del paquete, de lo contrario se entender� que se ha recibido en perfecto estado y no procederá la reclamación de la misma."
...
<table align="center" >
<tr>
	<td class="ref"> <? echo $row['ref'] ?> </td><td></td>
	</tr>
<tr>
<?php
foreach($articles as $row)
...
<tr>
<td><a href='borrar_pedidos.php?ref=<? echo $ref ?>' >BORRAR  </a> </td><td></td>
...
?>		 
<td align='left'>	<a href='<? $PHP_SELF ?>?imprimir=0&ref=<? echo $_GET['ref']; ?>&activar_re=2'  >
Desactivar RE</a> </td>
<?php
}
...
?>
<tr bgcolor="#CBCED8">
<td><? echo $row['nombre'] ?></td><td><? echo $row['precio'] ?> �</td>
</tr>
<?php
}
?>
